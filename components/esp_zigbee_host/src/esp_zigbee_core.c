...
static const char *TAG = "ESP_ZB_CORE";

/* No-op in NCP host mode: commissioning (network formation, steering) is handled
 * by the NCP firmware via ZNSP commands. The host dispatches signals directly
 * from esp_zb_init() and form_network_fn instead of going through BDB. */
...
esp_err_t esp_zb_set_primary_network_channel_set(uint32_t channel_mask)
{
    ...
    return esp_host_zb_output(ESP_ZNSP_NETWORK_PRIMARY_CHANNEL_SET, &channel_mask, sizeof(uint32_t), &output, &outlen);
}
...
void esp_zb_init(esp_zb_cfg_t *nwk_cfg)
{
    ...
    esp_err_t err = esp_host_zb_output(ESP_ZNSP_NETWORK_INIT, NULL, 0, &output, &outlen);
    if (err != ESP_OK) {
    ...
    esp_zb_app_signal_t app_signal = {
        ...
        .esp_err_status = (err == ESP_OK && output == ESP_ZNSP_SUCCESS) ? ESP_OK : ESP_ERR_INVALID_ARG,
    };

    esp_zb_app_signal_handler(&app_signal);

    err = esp_host_zb_output(ESP_ZNSP_NETWORK_FORMNETWORK, nwk_cfg, sizeof(esp_zb_cfg_t), &output, &outlen);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "NETWORK_FORMNETWORK failed: %s", esp_err_to_name(err));
    }
}
...
esp_err_t esp_zb_start(bool autostart)
{
    ...
    return esp_host_zb_output(ESP_ZNSP_NETWORK_START, &autostart, sizeof(bool), &output, &outlen);
}
...
esp_err_t esp_zb_ncp_get_network_state(void)
{
    ...
    return esp_host_zb_output(ESP_ZNSP_NETWORK_STATE, NULL, 0, &output, &outlen);
}
