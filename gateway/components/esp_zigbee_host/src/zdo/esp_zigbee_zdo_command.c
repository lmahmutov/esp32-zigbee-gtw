...
void esp_zb_zdo_active_ep_req(esp_zb_zdo_active_ep_req_param_t *cmd_req, esp_zb_zdo_active_ep_callback_t user_cb, void *user_ctx)
{
    ...
    esp_host_zb_output(ESP_ZNSP_ZDO_ACTIVE_EP, &zdo_data, sizeof(esp_zb_zdo_active_ep_t), &output, &outlen);
}
...
void esp_zb_zdo_simple_desc_req(esp_zb_zdo_simple_desc_req_param_t *cmd_req, esp_zb_zdo_simple_desc_callback_t user_cb, void *user_ctx)
{
    ...
    typedef struct {
        ...
        uint8_t endpoint;
        esp_zb_user_cb_t user_cb;
    } __attribute__((packed)) esp_zb_zdo_simple_desc_t;

    esp_zb_zdo_simple_desc_t zdo_data = {
        .addr = cmd_req->addr_of_interest,
    ...
    esp_host_zb_output(ESP_ZNSP_ZDO_SIMPLE_DESC, &zdo_data, sizeof(esp_zb_zdo_simple_desc_t), &output, &outlen);
}
