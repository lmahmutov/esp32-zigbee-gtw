...
/* Macros needed by sub-headers (must be before their includes) */
#define ESP_ZB_TRANSCEIVER_ALL_CHANNELS_MASK 0x07FFF800U /*!< channel 11-26 for compatibility with 2.4GHZ*/
#define ESP_ZB_NWK_BROADCAST_RX_ON_WHEN_IDLE  0xFFFD
#define ESP_ZB_TIME_ONE_SECOND  1000

#include "zcl/esp_zigbee_zcl_command.h"
#include "zdo/esp_zigbee_zdo_command.h"

/****************** ZCL-dependent types (need ZCL headers above) *********************/

/**
 * @brief ZCL attribute data structure
 */
typedef struct esp_zb_zcl_attribute_data_s {
    uint8_t type;               /*!< Attribute type */
    uint8_t size;               /*!< Attribute data size */
    void *value;                /*!< Attribute data value */
} esp_zb_zcl_attribute_data_t;

/**
 * @brief ZCL attribute structure (for report and read response)
 */
typedef struct esp_zb_zcl_attribute_s {
    uint16_t id;                                /*!< Attribute ID */
    esp_zb_zcl_attribute_data_t data;           /*!< Attribute data */
} esp_zb_zcl_attribute_t;

/**
 * @brief ZCL command info structure (header for all ZCL notifications)
 */
typedef struct esp_zb_zcl_cmd_info_s {
    esp_zb_zcl_status_t status;         /*!< Status */
    esp_zb_zcl_addr_t src_address;      /*!< Source address */
    uint8_t src_endpoint;               /*!< Source endpoint */
    uint8_t dst_endpoint;               /*!< Destination endpoint */
    uint16_t cluster;                   /*!< Cluster ID */
    uint16_t profile;                   /*!< Profile ID */
    uint8_t command;                    /*!< Command direction */
} ESP_ZB_PACKED_STRUCT esp_zb_zcl_cmd_info_t;

/**
 * @brief ZCL report attribute message
 */
...
/**
 * @brief ZCL read attr response variable (linked list)
 */
...
/**
 * @brief ZCL read attribute response message
 */
...
/**
 * @brief Host endpoint descriptor for NCP endpoint creation
 */
typedef struct {
    uint8_t     endpoint;
    uint16_t    profileId;
...
/**
 * @brief Type of the Zigbee core action handler callback
 */
typedef esp_err_t (*esp_zb_core_action_handler_t)(esp_zb_core_action_callback_id_t callback_id, const void *message);

/**
 * @brief Register the Zigbee core action handler.
 *
 * @param[in] cb  A callback that will be called when a ZCL action occurs
 */
void esp_zb_core_action_handler_register(esp_zb_core_action_handler_t cb);

/**
 * @brief Acquire the Zigbee stack lock (recursive mutex).
 *
 * @param[in] block_ticks  Maximum time to wait for the lock in ticks
 * @return true if the lock was acquired, false otherwise
 */
bool esp_zb_lock_acquire(uint32_t block_ticks);

/**
 * @brief Release the Zigbee stack lock.
 */
void esp_zb_lock_release(void);

/**
 * @brief Open the Zigbee network for joining.
 *
 * @param[in] duration  Permit join duration in seconds (0 = close, 0xFF = open indefinitely)
 * @return ESP_OK on success
 */
esp_err_t esp_zb_bdb_open_network(uint8_t duration);

/**
 * @brief Check whether the device is factory new.
 * @return true if factory new, false otherwise
 */
bool esp_zb_bdb_is_factory_new(void);

/**
 * @brief Ping the NCP by querying network state.
 *
 * Sends ESP_ZNSP_NETWORK_STATE to the NCP and returns the result.
 * Useful as a health check / watchdog ping.
 *
 * @return ESP_OK on success, ESP_ERR_TIMEOUT if NCP is unresponsive
 */
esp_err_t esp_zb_ncp_get_network_state(void);

/* Security stubs (handled on NCP side) */
...
/* Platform config types are defined in zb_config_platform.h (included via this header) */
