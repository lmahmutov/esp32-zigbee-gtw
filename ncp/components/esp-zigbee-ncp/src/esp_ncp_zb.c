...
static void esp_ncp_zb_zdo_scan_complete_handler(esp_zb_zdp_status_t zdo_status, uint8_t count, esp_zb_network_descriptor_t *nwk_descriptor)
{
    ...
    if (output) {
        ...
        if (nwk_descriptor && count) {
            memcpy(output + sizeof(esp_ncp_zb_scan_parameters_t), nwk_descriptor, (count * sizeof(esp_zb_network_descriptor_t)));
        }
...
static esp_err_t esp_ncp_zb_read_attr_resp_handler(const esp_zb_zcl_cmd_read_attr_resp_message_t *message, uint8_t **output, uint16_t *outlen)
{
    ...
    ESP_RETURN_ON_FALSE(message->info.status == ESP_ZB_ZCL_STATUS_SUCCESS, ESP_ERR_INVALID_ARG, TAG, "Received message: error status(%d)",
                        message->info.status);
    ESP_LOGI(TAG, "Read attribute response: status(%d), cluster(0x%x), src 0x%04x",
             message->info.status, message->info.cluster, message->info.src_address.u.short_addr);

    /* Wire header — packed to avoid struct padding mismatch between RISC-V (NCP) and Xtensa (host) */
    typedef struct {
        ...
        uint16_t cluster;
    } ESP_NCP_ZB_PACKED_STRUCT read_resp_hdr_t;

    /* Count attributes and total value bytes */
    uint8_t attr_count = 0;
    uint16_t attr_data_len = 0;
    for (esp_zb_zcl_read_attr_resp_variable_t *v = message->variables; v != NULL; v = v->next) {
        attr_data_len += 2 + 1 + 1 + (uint16_t)v->attribute.data.size;  /* id(2) + type(1) + size(1) + value */
        attr_count++;
    }

    uint16_t hdr_len = sizeof(read_resp_hdr_t);
    uint16_t length = hdr_len + 1 + attr_data_len;  /* header + count + attrs */
    uint8_t *outbuf = calloc(1, length);
    if (!outbuf) {
        return ESP_ERR_NO_MEM;
    }

    /* Serialize header field-by-field into packed struct */
    read_resp_hdr_t hdr = {
        .status       = message->info.status,
        .src_address  = message->info.src_address,
        .src_endpoint = message->info.src_endpoint,
        .dst_endpoint = message->info.dst_endpoint,
        .cluster      = message->info.cluster,
    };
    memcpy(outbuf, &hdr, hdr_len);
    outbuf[hdr_len] = attr_count;

    /* Serialize each attribute: id(2) + type(1) + size(1) + value(size) */
    uint8_t *p = outbuf + hdr_len + 1;
    for (esp_zb_zcl_read_attr_resp_variable_t *v = message->variables; v != NULL; v = v->next) {
        ESP_LOGI(TAG, "  attr(0x%x), type(0x%x), size(%d)", v->attribute.id, v->attribute.data.type, v->attribute.data.size);
        memcpy(p, &v->attribute.id, 2); p += 2;
        *p++ = v->attribute.data.type;
        uint8_t sz = (uint8_t)v->attribute.data.size;
        *p++ = sz;
        if (v->attribute.data.value && sz > 0) {
            memcpy(p, v->attribute.data.value, sz);
        }
        p += sz;
    }
...
static esp_err_t esp_ncp_zb_report_attr_handler(const esp_zb_zcl_report_attr_message_t *message, uint8_t **output, uint16_t *outlen)
{
    ...
    typedef struct {
        ...
        uint8_t dst_endpoint;
        uint16_t cluster;
    } ESP_NCP_ZB_PACKED_STRUCT report_hdr_t;

    typedef struct {
        uint16_t id;
        uint8_t  type;
        uint8_t  size;
    } ESP_NCP_ZB_PACKED_STRUCT report_attr_t;

    uint16_t hdr_len = sizeof(report_hdr_t);
    uint16_t attr_hdr_len = sizeof(report_attr_t);
    uint16_t length = hdr_len + attr_hdr_len + message->attribute.data.size;
    uint8_t *outbuf = calloc(1, length);
    if (!outbuf) return ESP_ERR_NO_MEM;

    /* Serialize header from message fields — do NOT memcpy from message struct */
    report_hdr_t hdr = {
        .status       = message->status,
        .src_address  = message->src_address,
        .src_endpoint = message->src_endpoint,
        .dst_endpoint = message->dst_endpoint,
    ...
    report_attr_t attr = {
        ...
        .type = message->attribute.data.type,
        .size = (uint8_t)message->attribute.data.size,
    };
    memcpy(outbuf + hdr_len, &attr, attr_hdr_len);

    if (message->attribute.data.value && message->attribute.data.size > 0) {
        memcpy(outbuf + hdr_len + attr_hdr_len, message->attribute.data.value, message->attribute.data.size);
    }
...
static esp_err_t esp_ncp_zb_add_endpoint_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    if (ncp_endpoint->inputClusterCount || ncp_endpoint->outputClusterCount) {
        ...
        if (attr_field) {
            for (int i = 0; i < zb_write_attr->attr_number; i ++) {
                ...
                if (attr_field[i].data.value) {
                    memcpy(attr_field[i].data.value, (uint8_t *)attr_data + attr_head_len, attr_data->size);
                } else {
                    ret = ESP_ERR_NO_MEM;
                    status = ESP_NCP_ERR_FATAL;
                    break;
...
static esp_err_t esp_ncp_zb_zcl_read_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    } ESP_NCP_ZB_PACKED_STRUCT esp_ncp_zb_zcl_read_t;

    esp_err_t ret = input ? ESP_OK : ESP_ERR_INVALID_ARG;
    esp_ncp_status_t status = (ret == ESP_OK) ? ESP_NCP_SUCCESS : ESP_NCP_ERR_FATAL;

    if (input) {
        esp_ncp_zb_zcl_read_t *read_data = (esp_ncp_zb_zcl_read_t *)input;
        uint16_t *attr_field = (uint16_t *)(input + sizeof(esp_ncp_zb_zcl_read_t));

        ESP_LOGI(TAG, "ZCL read: addr 0x%04x, ep %d->%d, cluster 0x%04x, attrs %d",
                 read_data->zcl_basic_cmd.dst_addr_u.addr_short,
                 read_data->zcl_basic_cmd.src_endpoint,
                 read_data->zcl_basic_cmd.dst_endpoint,
                 read_data->cluster_id, read_data->attr_number);

        esp_zb_zcl_read_attr_cmd_t cmd = {
            .zcl_basic_cmd = read_data->zcl_basic_cmd,
            .address_mode  = read_data->address_mode,
            .clusterID     = read_data->cluster_id,
            .attr_number   = read_data->attr_number,
            .attr_field    = attr_field,
        };
...
/* --- Permit Join handler (fixes NULL handler for 0x0005) --- */
static esp_err_t esp_ncp_zb_permit_joining_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    esp_err_t ret = ESP_OK;
    esp_ncp_status_t status = ESP_NCP_ERR_FATAL;

    if (input && inlen >= sizeof(uint8_t)) {
        uint8_t duration = *input;
        ESP_LOGI(TAG, "Permit joining for %d seconds", duration);
        ret = esp_zb_bdb_open_network(duration);
        status = (ret == ESP_OK) ? ESP_NCP_SUCCESS : ESP_NCP_ERR_FATAL;
    }
...
/* --- Active EP request handler (new 0x0203) --- */
typedef struct {
    uint16_t                addr;
    esp_ncp_zb_user_cb_t   user_cb;
} ESP_NCP_ZB_PACKED_STRUCT esp_ncp_zb_active_ep_req_t;

static void esp_ncp_zb_active_ep_cb(esp_zb_zdp_status_t zdo_status, uint8_t ep_count, uint8_t *ep_id_list, void *user_ctx)
{
    esp_ncp_header_t ncp_header = {
        .sn = esp_random() % 0xFF,
        .id = ESP_NCP_ZDO_ACTIVE_EP,
    };

    /* Response: status(1) + addr(2) + ep_count(1) + ep_list(ep_count) + user_cb */
    uint16_t outlen = sizeof(uint8_t) + sizeof(uint16_t) + sizeof(uint8_t) + ep_count + sizeof(esp_ncp_zb_user_cb_t);
    uint8_t *output = calloc(1, outlen);
    if (output) {
        uint8_t *p = output;
        *p++ = zdo_status;

        /* Retrieve the address from user_ctx */
        esp_ncp_zb_active_ep_req_t *req = (esp_ncp_zb_active_ep_req_t *)user_ctx;
        memcpy(p, &req->addr, sizeof(uint16_t));
        p += sizeof(uint16_t);

        *p++ = ep_count;
        if (ep_count && ep_id_list) {
            memcpy(p, ep_id_list, ep_count);
            p += ep_count;
        }
        memcpy(p, &req->user_cb, sizeof(esp_ncp_zb_user_cb_t));
...
static esp_err_t esp_ncp_zb_active_ep_req_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    esp_err_t ret = ESP_OK;
    esp_ncp_status_t status = ESP_NCP_ERR_FATAL;

    if (input && inlen >= sizeof(esp_ncp_zb_active_ep_req_t)) {
        esp_ncp_zb_active_ep_req_t *req_in = (esp_ncp_zb_active_ep_req_t *)input;

        /* Store request info for the callback */
        esp_ncp_zb_active_ep_req_t *user_ctx = calloc(1, sizeof(esp_ncp_zb_active_ep_req_t));
        if (user_ctx) {
            memcpy(user_ctx, req_in, sizeof(esp_ncp_zb_active_ep_req_t));

            esp_zb_zdo_active_ep_req_param_t cmd_req = {
                .addr_of_interest = req_in->addr,
            };
            esp_zb_zdo_active_ep_req(&cmd_req, esp_ncp_zb_active_ep_cb, user_ctx);
            status = ESP_NCP_SUCCESS;
        } else {
            ret = ESP_ERR_NO_MEM;
        }
...
/* --- Simple Desc request handler (new 0x0204) --- */
typedef struct {
    uint16_t                addr;
    uint8_t                 endpoint;
    esp_ncp_zb_user_cb_t   user_cb;
} ESP_NCP_ZB_PACKED_STRUCT esp_ncp_zb_simple_desc_req_t;

static void esp_ncp_zb_simple_desc_cb(esp_zb_zdp_status_t zdo_status, esp_zb_af_simple_desc_1_1_t *simple_desc, void *user_ctx)
{
    esp_ncp_header_t ncp_header = {
        .sn = esp_random() % 0xFF,
        .id = ESP_NCP_ZDO_SIMPLE_DESC,
    };

    esp_ncp_zb_simple_desc_req_t *req = (esp_ncp_zb_simple_desc_req_t *)user_ctx;

    if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS && simple_desc) {
        uint8_t total_clusters = simple_desc->app_input_cluster_count + simple_desc->app_output_cluster_count;
        /* Response: status(1) + addr(2) + endpoint(1) + profile_id(2) + device_id(2) +
           device_version(1) + input_cluster_count(1) + output_cluster_count(1) + cluster_list(total*2) + user_cb */
        uint16_t outlen = 1 + 2 + 1 + 2 + 2 + 1 + 1 + 1 + (total_clusters * sizeof(uint16_t)) + sizeof(esp_ncp_zb_user_cb_t);
        uint8_t *output = calloc(1, outlen);
        if (output) {
            uint8_t *p = output;
            *p++ = zdo_status;
            memcpy(p, &req->addr, sizeof(uint16_t)); p += 2;
            *p++ = simple_desc->endpoint;
            memcpy(p, &simple_desc->app_profile_id, sizeof(uint16_t)); p += 2;
            memcpy(p, &simple_desc->app_device_id, sizeof(uint16_t)); p += 2;
            *p++ = simple_desc->app_device_version;
            *p++ = simple_desc->app_input_cluster_count;
            *p++ = simple_desc->app_output_cluster_count;
            if (total_clusters && simple_desc->app_cluster_list) {
                memcpy(p, simple_desc->app_cluster_list, total_clusters * sizeof(uint16_t));
                p += total_clusters * sizeof(uint16_t);
            }
            memcpy(p, &req->user_cb, sizeof(esp_ncp_zb_user_cb_t));
        ...
        /* Error response: status + addr + user_cb */
        uint16_t outlen = 1 + 2 + sizeof(esp_ncp_zb_user_cb_t);
        uint8_t *output = calloc(1, outlen);
        if (output) {
            uint8_t *p = output;
            *p++ = zdo_status;
...
static esp_err_t esp_ncp_zb_simple_desc_req_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    esp_err_t ret = ESP_OK;
    esp_ncp_status_t status = ESP_NCP_ERR_FATAL;

    if (input && inlen >= sizeof(esp_ncp_zb_simple_desc_req_t)) {
        esp_ncp_zb_simple_desc_req_t *req_in = (esp_ncp_zb_simple_desc_req_t *)input;

        esp_ncp_zb_simple_desc_req_t *user_ctx = calloc(1, sizeof(esp_ncp_zb_simple_desc_req_t));
        if (user_ctx) {
            memcpy(user_ctx, req_in, sizeof(esp_ncp_zb_simple_desc_req_t));

            esp_zb_zdo_simple_desc_req_param_t cmd_req = {
                .addr_of_interest = req_in->addr,
                .endpoint = req_in->endpoint,
            };
            esp_zb_zdo_simple_desc_req(&cmd_req, esp_ncp_zb_simple_desc_cb, user_ctx);
            status = ESP_NCP_SUCCESS;
        } else {
            ret = ESP_ERR_NO_MEM;
        }
...
static const esp_ncp_zb_func_t ncp_zb_func_table[] = {
    ...
    {ESP_NCP_NETWORK_PERMIT_JOINING, esp_ncp_zb_permit_joining_fn},
    ...
    {ESP_NCP_ZDO_ACTIVE_EP, esp_ncp_zb_active_ep_req_fn},
    {ESP_NCP_ZDO_SIMPLE_DESC, esp_ncp_zb_simple_desc_req_fn},
...
esp_err_t esp_ncp_zb_output(esp_ncp_header_t *ncp_header, const void *buffer, uint16_t len)
{
    ...
    for (int i = 0; i < sizeof(ncp_zb_func_table) / sizeof(ncp_zb_func_table[0]); i ++) {
        ...
        if (ncp_zb_func_table[i].set_func) {
            /* APS data indication/confirm only poll queues — no esp_zb_* calls,
               must NOT hold the lock (would stall the stack during the 100 ms wait) */
            bool need_lock = (ncp_header->id != ESP_NCP_APS_DATA_INDICATION &&
                              ncp_header->id != ESP_NCP_APS_DATA_CONFIRM);
            if (need_lock) esp_zb_lock_acquire(portMAX_DELAY);
            ret = ncp_zb_func_table[i].set_func(buffer, len, &output, &outlen);
            if (need_lock) esp_zb_lock_release();
            if (ret == ESP_OK) {
